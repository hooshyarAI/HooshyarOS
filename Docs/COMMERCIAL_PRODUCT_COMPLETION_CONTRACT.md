...
with `schemaVersion: 1` and `status: "VERIFIED"`.

The manifest MUST be generated by an executed verification process and MUST bind itself to the exact repository commit under test. The audit must reject missing, malformed, unverified or commit-mismatched evidence.

The manifest MUST contain successful command evidence (`exitCode: 0`) and explicit runtime/application evidence for:

- installer produced, with artifact path, SHA-256 and non-zero size
...
A source file, a unit test, a mocked process, a build exit code, a screenshot, or a textual claim MUST NOT be substituted for executed application/acceptance evidence.

If an external dependency is genuinely unavailable, the manifest MUST record it as `required: true, status: "BLOCKED"` with its exact name. It must never be recorded as verified merely because a local stub exists.
