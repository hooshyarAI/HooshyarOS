...
for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
  ...
  const evidenceExit = run(executable('npm'), ['run', 'product:cline:evidence']);
  ...
  if ((factory.status ?? 1) === 0 && currentEvidence?.verdict === 'QUALIFICATION_COMPLETE') {
    ...
    console.log(JSON.stringify({ type: 'AUTONOMOUS_FACTORY_COMPLETE', attempt, commit: after, evidenceExitCode: evidenceExit.status ?? 1 }));
    process.exit(0);
  }
