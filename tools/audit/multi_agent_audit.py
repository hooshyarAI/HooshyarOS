...
RUNTIME_DIRS={'Core','Engines','Registry','Services','Entities'}
...
def repository_evidence(root:Path)->dict[str,Any]:
    ...
    for p in root.rglob('*'):
        if p.is_file() and not any(x in EXCLUDED for x in p.parts) and p.suffix.lower() in TEXT_EXTENSIONS:
            rel=p.relative_to(root).as_posix();paths.append(rel)
            parts=p.parts
            if len(parts)>=3 and parts[0]=='Backend' and parts[1]=='HBOS' and parts[2] in RUNTIME_DIRS and p.suffix.lower()=='.ts':
                runtime_symbols[p.stem].append(rel)
    roadmap=root/'Docs/Product/PRODUCT_CONSTRUCTION_ROADMAP.json';data={}
    if roadmap.exists():
    ...
    duplicate_runtime={k:sorted(v) for k,v in runtime_symbols.items() if len({Path(x).parts[2] for x in v})>1}
    return {'commit':git(root,'rev-parse','HEAD'),'branch':git(root,'branch','--show-current'),'status':git(root,'status','--porcelain=v1','--untracked-files=all'),'fileCount':len(paths),'documentationCount':sum(p.lower().startswith('docs/') for p in paths),'capabilityIds':sorted(ids),'duplicateCapabilityIds':dup,'duplicateRuntimeSymbols':duplicate_runtime,'masterCharterPresent':(root/'Docs/HOOSHYAROS_MASTER_CHARTER.md').exists(),'architecturePresent':(root/'Docs/ARCHITECTURE.md').exists(),'governanceCharterPresent':(root/'Docs/HOOSHYAROS_GOVERNANCE_CHARTER.md').exists()}
...
def deterministic(e:dict[str,Any])->list[dict[str,Any]]:
    ...
    if e['duplicateRuntimeSymbols']:
        f.append({'id':'RUNTIME_SYMBOL_DUPLICATE','severity':'CRITICAL','claim':'Runtime symbols must have one canonical implementation across HBOS runtime layers','evidence':e['duplicateRuntimeSymbols']})
    return f
...
def load_reports(root:Path)->tuple[list[dict[str,Any]],list[dict[str,Any]]]:
    ...
    for auditor in EXTERNAL_AUDITORS:
        ...
        if not path.exists():defects.append({'id':'EXTERNAL_EVIDENCE_MISSING','severity':'HIGH','auditor':auditor,'path':str(path.relative_to(root))});continue
        try:r=json.loads(read(path))
        except json.JSONDecodeError:defects.append({'id':'EXTERNAL_EVIDENCE_INVALID_JSON','severity':'HIGH','auditor':auditor});continue
        if not isinstance(r,dict) or not REQUIRED_FIELDS.issubset(r):defects.append({'id':'EXTERNAL_EVIDENCE_SCHEMA_INVALID','severity':'HIGH','auditor':auditor});continue
        if r.get('auditor')!=auditor or r.get('auditor') not in ALLOWED_AUDITORS:defects.append({'id':'EXTERNAL_AUDITOR_UNAUTHORIZED','severity':'CRITICAL','auditor':r.get('auditor')});continue
        if not isinstance(r.get('findings'),list):defects.append({'id':'EXTERNAL_FINDINGS_INVALID','severity':'HIGH','auditor':auditor});continue
        if not isinstance(r.get('commit'),str) or not r['commit']:defects.append({'id':'EXTERNAL_COMMIT_MISSING','severity':'HIGH','auditor':auditor});continue
        reports.append(r)
