...
try {
  ...
  $Archive = Join-Path $Root "HooshyarOS-Windows-Bootstrap.zip"
  if (!(Test-Path $Archive)) { throw "Bootstrap archive missing: $Archive" }
  Expand-Archive -LiteralPath $Archive -DestinationPath $RuntimeRoot -Force
  "INSTALLED $(Get-Date -Format o)" | Add-Content -Encoding UTF8 $Log
