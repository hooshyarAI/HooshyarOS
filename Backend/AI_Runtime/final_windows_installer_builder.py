...
const routes = {
  "/": ["text/html; charset=utf-8", "<!doctype html><html lang=\"fa\" dir=\"rtl\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Hooshyar.ai</title><style>body{font-family:Segoe UI,Tahoma,sans-serif;background:#f6f8fb;margin:0;color:#172033}main{max-width:960px;margin:9vh auto;padding:40px;background:white;border-radius:20px;box-shadow:0 15px 50px rgba(20,40,80,.12)}h1{margin-top:0;font-size:42px}p{font-size:20px;color:#526078}.ok{margin-top:24px;padding:14px 18px;background:#eefaf2;color:#166534;border-radius:12px}</style></head><body><main><h1>هوشیار.ai</h1><p>Enterprise Intelligence Platform</p><div class=\"ok\">Windows desktop runtime is ready.</div></main></body></html>"],
  ...
  "/health": ["application/json; charset=utf-8", JSON.stringify({status:"ok",state:"ready"})]
};
...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            // Important: do not kill the packaged Node runtime here.
            // It must remain alive for the desktop application after this shell exits.
    ...
    private static bool WaitUntilReadyAsync()
    {
...
def build_desktop_shell(dotnet: str) -> None:
    with tempfile.TemporaryDirectory(prefix="hooshyar-shell-") as temp:
        ...
        out = root / "publish"
        result = subprocess.run(
            [dotnet, "publish", str(project), "-c", "Release", "-r", "win-x64", "--self-contained", "true", "-p:UseAppHost=true", "-p:PublishSingleFile=true", "-o", str(out), "--nologo"],
            cwd=root, env=env, text=True, encoding="utf-8", errors="replace", stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=30 * 60, check=False,
        )
        print(result.stdout, end="")
        if result.returncode != 0:
