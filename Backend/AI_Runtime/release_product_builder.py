...
def ensure_android_toolchain() -> tuple[Path, Path, Path]:
    ...
    subprocess.run([str(sdkmanager), "platform-tools", "platforms;android-35", "build-tools;34.0.0"], cwd=ROOT, env=env, check=True)
    subprocess.run([str(sdkmanager), "--licenses"], cwd=ROOT, env=env, input="y\n" * 64, text=True, check=False)
...
def windows() -> int:
    ...
    for name in ("Backend", "Frontend", "node_modules"):
        src = ROOT / name
        if src.exists(): shutil.copytree(src, payload / name, dirs_exist_ok=True)
    runtime = payload / "runtime"
    runtime.mkdir(parents=True, exist_ok=True)
    shutil.copy2(node, runtime / "node.exe")
    install_inner = source / "install.ps1"
    install_inner.write_text(r'''$ErrorActionPreference="Stop"
...
''', encoding="utf-8")
    ...
    with zipfile.ZipFile(package, "w", zipfile.ZIP_DEFLATED) as zf:
        for p in payload.rglob("*"):
            if p.is_file(): zf.write(p, p.relative_to(source))
    ...
    if not iexpress.exists(): raise RuntimeError("IExpress is unavailable")
    ...
    if result.returncode or not target.exists(): raise RuntimeError("IExpress did not produce HooshyarOS-Setup.exe")
...
def android() -> int:
    ...
    if not apks: raise RuntimeError("Android build produced no APK")
