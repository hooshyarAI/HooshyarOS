...
class AutonomousMasterOrchestratorTests(unittest.TestCase):
    ...
    def test_verifies_and_publishes_a_built_cycle(self):
        with patch.object(orchestrator, "git_clean", side_effect=[True, True]), patch.object(
            orchestrator, "run_builder", return_value=True
        ), patch.object(orchestrator, "git_porcelain", side_effect=[" M Backend/HBOS/Product/example.ts"]), patch.object(
            orchestrator, "verify", side_effect=[True, True]
