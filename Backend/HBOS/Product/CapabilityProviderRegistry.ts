...
export const HOOSHYAROS_CAPABILITY_PROVIDER_INVENTORY: ReadonlyArray<CapabilityProviderCandidate> = [
  ...
  },
  {
    ...
    status: "APPROVED",
    integration: "INTEGRATED",
    declaredDependency: "tesseract.js",
    license: "Apache-2.0",
    licenseEvidence: "node_modules/tesseract.js/package.json#license and node_modules/tesseract.js-core/package.json#license are both Apache-2.0; language data packages @tesseract.js-data/eng and @tesseract.js-data/fas are MIT over Apache-2.0 tessdata_fast data (verified in-repo 2026-09-21; see .kilo/evidence/ocr-provider-admission-2026-09-21.md).",
    commercialUseCompatible: true,
    selfHostedOffline: true,
    maintenance: {
      source: "node_modules/tesseract.js/package.json (installed 7.0.0) + node_modules/tesseract.js-core/package.json (installed 7.0.0) + @tesseract.js-data/eng|fas (installed 1.0.0)",
      asOf: "2026-09-21",
      note: "Presence, pinned versions and licenses verified in-repo. The WASM core and the language data are read from declared local dependencies; the adapter sets cacheMethod 'none' with a local langPath, so there is no runtime CDN fetch.",
    },
    securityConsiderations: "WASM OCR engine processing untrusted rasterized images in-process with no network access and no code execution. Product/PdfPageRasterizer bounds page count and per-page byte size before OCR; Product/ScannedPdfRouter bounds per-page OCR time; language data is staged from declared local packages only, so a CDN fetch cannot occur. OCR text is normalized through the canonical validation pipeline and cannot bypass it.",
    replaceableVia: "Product/OcrAdapter.ts (createCanonicalOcrAdapter + OcrAdapter contract) behind Product/ScannedPdfRouter.ts and Product/FinancialIngestionService.ts",
    provenanceResponsibility: "HooshyarOS owns OCR provenance: the canonical model's SHA-256 stays the ORIGINAL PDF-byte hash, and engine identity/version/confidence/language is recorded via Product/OcrProvenance.ts.",
    testingResponsibility: "HooshyarOS owns the focused OCR tests (OcrAdapter, OcrProviderAdmission, PdfOcrRouting, PdfIngestionRuntime) plus the real-node PDF ingestion acceptance script.",
    upgradeResponsibility: "The engine and language-data packages are pinned in package.json; upgrades re-run the focused OCR tests and the PDF ingestion acceptance script.",
    deprecationHandling: "The capability is behind the OcrAdapter contract, so the provider (or the language-data source) can be replaced without touching canonical ingestion, validation or provenance.",
    reason: "tesseract.js is a mature Apache-2.0 OCR engine that runs fully offline through a WASM core and declared local language-data packages, closing the honest remaining PDF limitation (scanned/image-only PDFs) instead of hiding it.",
    decidedBy: "hooshyaros-governance",
  ...
  },
  {
    ...
    maintenance: {
      ...
      asOf: "2026-09-19",
      note: "Presence, pinned version and license verified in-repo; the rasterizer is reached through the already-admitted pdf-parse dependency, so no new runtime dependency is introduced. Its bundled render stack (@napi-rs/canvas) is additionally declared as a devDependency for the scanned-PDF test/acceptance fixtures.",
    },
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    provider: "xls-reader",
    purpose: "Read legacy binary .xls (OLE2 / BIFF8, Excel 97-2003) workbooks into the same canonical document/statement normalization boundary used by XLSX and PDF.",
    reuseTier: "MATURE_LIBRARY",
    status: "APPROVED",
    integration: "INTEGRATED",
    declaredDependency: "xls-reader",
    license: "MIT",
    licenseEvidence: "package.json \"license\": \"MIT\" plus upstream LICENSE at https://github.com/zanlucathiago/xls-reader (MIT © Thiago Zanluca); verified asOf 2026-09-22.",
    commercialUseCompatible: true,
    selfHostedOffline: true,
    maintenance: {
      source: "registry metadata for xls-reader (npm) and https://github.com/zanlucathiago/xls-reader",
      asOf: "2026-09-22",
      note: "Ten published versions (0.1.0 -> 0.7.0) between 2026-07-06 and 2026-07-20 with CI, coverage and npm provenance; actively maintained at admission time. Residual risk: young project, so the adapter boundary stays replaceable and the dependency is pinned exactly.",
    },
    securityConsiderations: "Zero runtime dependencies, pure Uint8Array/DataView parsing with no eval, no dynamic code, no network and no child process (verified by inspecting the published dist). Reads cached FORMULA values only and recognizes/skips chart, Excel-4 macro and VBA substreams, so no macro or formula executes. Encrypted workbooks are rejected by the provider. Malformed/truncated input is bounded by the existing XLS size limit and fail-closed error mapping.",
    replaceableVia: "FinancialDataIngestionAdapter.ingestXlsBytes behind the Product/FinancialDocumentUnderstanding boundary; the provider only yields cell values.",
    provenanceResponsibility: "HooshyarOS owns the original-byte SHA-256, tenant scope and canonical model; the provider only returns cells.",
    testingResponsibility: "HooshyarOS owns focused XLS tests (real BIFF8 fixture, multi-sheet statement workbook, ledger workbook, malformed/truncated rejection). PASS requires a real reader.",
    upgradeResponsibility: "xls-reader is pinned exactly in package.json and re-verified on upgrade.",
    deprecationHandling: "Replace behind ingestXlsBytes without changing the canonical model or the runtime contract.",
    reason: "SheetJS no longer publishes to npm (0.18.5 frozen with advisories) and exceljs-hardened does not read the legacy OLE2 .xls format; xls-reader is a maintained, MIT, zero-dependency, offline, npm-published reader that satisfies every admission criterion.",
    decidedBy: "hooshyaros-governance",
