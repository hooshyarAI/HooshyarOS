...
describe("CommercialDecisionExecutionWorkflow", () => {
  ...
  test("executes an observable side effect, persists evidence, and survives restart", async () => {
    ...
    const receipt = JSON.parse(readFileSync(result.executionReceiptPath, "utf8")) as { tenantId: string; decisionId: string; assignee: string; approvedBy: string; action: { action: string } };
    ...
    await expect(store.read({ tenantId: actor.tenantId }, "outcome:decision-42")).resolves.toMatchObject({ value: { variance: 0.15 } });
  ...
  test("requires an independent privileged approver from the same tenant", async () => {
    ...
    await expect(workflow.approveAndExecute({ token: actor.token, organization: "org-a", decisionId: "decision-denied", decision: { action: "unsafe" }, approverToken: otherManager.token, approverOrganization: "org-a", assignee: "finance-team" })).rejects.toThrow("APPROVAL_ROLE_REQUIRED");
    store.close();
    identity.close();
  });

  test("rejects a privileged approver from another tenant before execution", async () => {
    const identity = new CommercialIdentityService(join(directory, "identity.sqlite"));
    identity.initialize();
    const actor = identity.createSession("manager", "org-a", "MANAGER");
    const foreignApprover = identity.createSession("owner-b", "org-b", "OWNER");
    const store = new SQLitePersistenceStore({ databasePath: join(directory, "product.sqlite") });
    const executor = new FilesystemDecisionExecutor(join(directory, "execution"));
    const workflow = new CommercialDecisionExecutionWorkflow(new CommercialAuthorizationBoundary(identity), new CommercialPersistenceBoundary(store), executor);

    await expect(workflow.approveAndExecute({ token: actor.token, organization: "org-a", decisionId: "decision-cross-tenant", decision: { action: "unsafe" }, approverToken: foreignApprover.token, approverOrganization: "org-b", assignee: "finance-team" })).rejects.toThrow("APPROVAL_TENANT_SCOPE_MISMATCH");
    store.close();
    identity.close();
  });
});
