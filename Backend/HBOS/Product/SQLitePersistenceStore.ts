...
/** Canonical tenant-scoped SQLite persistence adapter used by product evidence. */
export class SQLitePersistenceStore {
  ...
  private assertScope(scope: TenantScope): void {
    if (!scope?.tenantId?.trim()) throw new Error("persistence-tenant-required");
  }

  private assertKey(key: string): void {
    if (!key?.trim()) throw new Error("persistence-key-required");
  }
}
