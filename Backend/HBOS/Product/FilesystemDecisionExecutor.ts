...
export class FilesystemDecisionExecutor {
  ...
  async execute(input: Omit<DecisionExecutionReceipt, "executedAt">): Promise<DecisionExecutionResult> {
    const receipt: DecisionExecutionReceipt = { ...input, executedAt: new Date().toISOString() };
    const receiptPath = join(this.rootDirectory, "tenants", input.tenantId, "executions", `${input.decisionId}.json`);
