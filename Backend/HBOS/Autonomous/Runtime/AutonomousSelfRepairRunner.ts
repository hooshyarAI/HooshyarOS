...
export class AutonomousSelfRepairRunner {
    ...
    private createStrategies(context: SelfRepairRunnerContext, capture: (result: AutonomousDevelopmentResult) => void): RepairStrategy[] {
        const make = (kind: RepairStrategyKind, description: string, risk: number, fit: number, reversibility: number): RepairStrategy => ({
            ...
            execute: () => {
                ...
                return {
                    ok: result.result.ok,
                    evidence: [`STRATEGY=${kind}`, `RESULT_STATUS=${result.status}`, `RESULT_STAGE=${result.result.stage}`, `REPOSITORY_CHANGED=${changed}`, `WORKTREE_CLEAN=${after.clean}`, ...(result.result.issues ?? [])],
