...
export class CommercialProductCompletionAudit {
    ...
    private hasWebApplicationEvidence(root: string): boolean {
        ...
        for (const path of testPaths) {
            ...
            const hasRuntimeImport = test.includes("CommercialRuntimeServer") || test.includes("createCommercialRuntimeServer");
            const hasBehaviorAssertions = ["request(", "expect(", "status"].every(marker => test.includes(marker));
            if (hasRuntimeImport && hasBehaviorAssertions) return true;
        }
        return false;
    }
