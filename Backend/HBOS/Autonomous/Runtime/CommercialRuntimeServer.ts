...
const ANALYSIS_KEY = "financial-analysis:latest";
...
const stableTenantId = (username: string, organization: string): string => {
    const identity = `${username.trim().toLowerCase()}\u0000${organization.trim()}`;
    return `tenant:${createHash("sha256").update(identity, "utf8").digest("hex").slice(0, 32)}`;
};

const dashboardFromResult = (result: AnalysisResult) => ({
...
export function createCommercialRuntimeServer(options: CommercialRuntimeOptions = {}): Server {
    ...
    const latestResults = new Map<string, AnalysisResult>();
    ...
    const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
        try {
            ...
            if (req.method === "POST" && path === "/api/analyze") {
                ...
                await persistence.write({ tenantId: session.tenantId }, ANALYSIS_KEY, result);
                return json(res, 200, result);
            }

            if (req.method === "GET" && path === "/api/dashboard") {
                const inMemory = latestResults.get(session.tenantId);
                if (inMemory) return json(res, 200, dashboardFromResult(inMemory));
                const persisted = await persistence.read({ tenantId: session.tenantId }, ANALYSIS_KEY);
                const result = persisted?.value as AnalysisResult | undefined;
                ...
                latestResults.set(session.tenantId, result);
                return json(res, 200, dashboardFromResult(result));
            }
