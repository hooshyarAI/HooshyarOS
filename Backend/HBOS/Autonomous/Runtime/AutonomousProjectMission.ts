...
export class AutonomousProjectMission {
    ...
    private capabilityBacklog(): CapabilityDefinition[] {
        ...
        return [
            ...
            { id: "platform.security-layer", capability: "implement the canonical Security Layer capability", targetEngine: "Security Layer Engine", dependencies: ["HBOS Core", "Governance Engine"], requiredPaths: [p("Backend/HBOS/Engines/SecurityLayerEngine.ts"),p("Backend/HBOS/test/SecurityLayerEngine.test.ts")], verificationPaths: focused("Backend/HBOS/test/SecurityLayerEngine.test.ts") },
            ...
            { id: "platform.production-readiness", capability: "implement repository-native Production Readiness capability", targetEngine: "Production Readiness Engine", dependencies: ["Reports Engine", "Security Layer Engine"], requiredPaths: [p("Backend/HBOS/Engines/ProductionReadinessEngine.ts"),p("Backend/HBOS/test/ProductionReadinessEngine.test.ts"),p("Docs/Engines/ProductionReadinessEngine.md")], verificationPaths: focused("Backend/HBOS/test/ProductionReadinessEngine.test.ts") },
    ...
    private isCapabilityImplemented(capability: CapabilityDefinition): boolean {
        const p = (file: string) => existsSync(file);
        const implementation = capability.requiredPaths.filter((_, i) => i === 0).every(p) || capability.requiredPaths.every(p);
        const test = capability.requiredPaths.slice(1).some(p);
        const documentation = capability.requiredPaths.some(path => path.endsWith(".md"));
        const supportingEvidence = !capability.evidencePaths || capability.evidencePaths.some(p);
        const verification = capability.verificationPaths?.every(p) ?? false;
