...
export class AutonomousProjectMission {
    ...
    nextPlatformMission(): Omit<Mission, "evidence" | "architectureRules" | "directives"> | null { const next = this.capabilityBacklog().find(capability => !this.isCapabilityImplemented(capability)); return next ? { capabilityId: next.id, capability: next.capability, targetEngine: next.targetEngine, dependencies: next.dependencies } : null; }
    ...
    private capabilityBacklog(): CapabilityDefinition[] {
        return [
            ...
            { id: "platform.user-management", capability: "implement the Phase 2 User Management capability", targetEngine: "User Management Engine", dependencies: ["HBOS Core", "Governance Engine"], requiredPaths: [join(this.root,"Backend/HBOS/Engines/UserManagementEngine.ts"),join(this.root,"Backend/HBOS/test/UserManagementEngine.test.ts")] },
            ...
            { id: "platform.security-layer", capability: "implement the Phase 2 Security Layer capability", targetEngine: "Security Layer Engine", dependencies: ["Governance Engine", "User Management Engine", "Organization Model Engine"], requiredPaths: [join(this.root,"Backend/HBOS/Engines/SecurityLayerEngine.ts"),join(this.root,"Backend/HBOS/test/SecurityLayerEngine.test.ts")] }
        ];
    }
    private isCapabilityImplemented(capability: CapabilityDefinition): boolean { const required = capability.requiredPaths.every(existsSync); return required && (!capability.evidencePaths || capability.evidencePaths.length === 0 || capability.evidencePaths.some(existsSync)); }
    private assistantCompletionEvidence(): boolean {
        ...
        const pythonOnly = runtime.includes('type ImplementationAgent = "python"') && runtime.includes("Backend/AI_Runtime/autonomous_builder.py");
        ...
        const coverage = this.capabilityBacklog().every(cap => builder.includes(`\"${cap.id}\"`));
        const local = builder.includes("argparse") && builder.includes("CAPABILITIES") && builder.includes("Capability ID:") && builder.includes("if not generated:") && coverage && !builder.includes("subprocess.run([\"copilot\"") && !builder.includes("subprocess.run([\"codex\"") && !builder.includes("subprocess.run([\"claude\"");
        ...
        return pythonOnly && lifecycle && orchestration && local && reasoningContract;
    }
