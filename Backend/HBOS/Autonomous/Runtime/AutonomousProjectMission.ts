...
interface CapabilityDefinition { id: string; capability: string; targetEngine: string; dependencies: string[]; requiredPaths: string[]; evidencePaths?: string[]; verificationPaths?: string[]; behaviorEvidence?: string[]; }
...
export class AutonomousProjectMission {
    ...
    private capabilityBacklog(): CapabilityDefinition[] {
        ...
        return [
            ...
            { id: "platform.api-gateway", capability: "implement the Phase 2 API Gateway capability", targetEngine: "API Gateway Engine", dependencies: ["Security Layer Engine"], requiredPaths: [p("Backend/HBOS/Engines/APIGatewayEngine.ts"),p("Backend/HBOS/test/APIGatewayEngine.test.ts"),p("Docs/Engines/APIGatewayEngine.md")], verificationPaths: focused("Backend/HBOS/test/APIGatewayEngine.test.ts"), behaviorEvidence: ["handleRequest", "request"] },
            ...
            { id: "platform.cloud-deployment", capability: "implement repository-native Cloud Deployment capability", targetEngine: "Cloud Deployment Engine", dependencies: ["Deployment Contract Engine"], requiredPaths: [p("Backend/HBOS/Engines/CloudDeploymentEngine.ts"),p("Backend/HBOS/test/CloudDeploymentEngine.test.ts"),p("Docs/Engines/CloudDeploymentEngine.md")], verificationPaths: focused("Backend/HBOS/test/CloudDeploymentEngine.test.ts"), behaviorEvidence: ["deploy(", "plan("] }
        ];
    }
    ...
    private isCapabilityImplemented(capability: CapabilityDefinition): boolean {
        ...
        const behavior = (capability.behaviorEvidence ?? []).length === 0 || capability.behaviorEvidence!.some(marker => {
            const enginePath = capability.requiredPaths.find(path => /Engines[\\/].+\.ts$/.test(path));
        ...
        return this.evidenceAudit.evaluate({ implementation, test, documentation, dependenciesSatisfied: dependenciesSatisfied && supportingEvidence, verified: verification && behavior }).complete;
    }
    ...
    private directives(): string[] { return ["Read Docs/ARCHITECTURE.md and existing engine implementations before changing code","Read Assistant/SYSTEM_PROMPT.md as the development constitution","Inspect the existing AI Runtime before creating a new implementation","Implement exactly ONE concrete capability from the canonical backlog","Create or update the focused implementation, focused test and documentation required by the architecture","Run focused verification followed by the full Jest suite","Repair verification failures before finalization","Do not stop at a plan or report; produce a real repository change when the selected capability is missing or incomplete","Do not redesign Architecture Freeze V4"]; }
