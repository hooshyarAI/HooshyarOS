...
export class AutonomousProjectMission {
    ...
    private isCapabilityImplemented(capability: CapabilityDefinition): boolean {
        if (!capability.requiredPaths.every(existsSync)) return false;
        return !capability.evidencePaths || capability.evidencePaths.length === 0 || capability.evidencePaths.some(existsSync);
    }
