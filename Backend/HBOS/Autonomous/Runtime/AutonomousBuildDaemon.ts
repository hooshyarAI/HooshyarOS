...
export class AutonomousBuildDaemon {
    ...
    constructor(options: DaemonOptions = {}) {
        ...
        this.development = options.development ?? new AutonomousDevelopmentLoop([]);
    ...
    private selectMission(): MissionDecision {
        ...
        if (commercialAudit.blockedExternalDependencies.length > 0) return { kind: "platform-audit-blocked", mission: selected, assistantGatePassed: true, continuation, reason: "BLOCKED_EXTERNAL_DEPENDENCY", details: { ...commercialAudit, terminal: true } };
    ...
    run() {
        ...
        for (let cycle = 1; cycle <= this.maxCycles; cycle += 1) {
            ...
            catch (error) { console.log(JSON.stringify({ type: "AUTONOMOUS_PERFORMANCE_BUDGET_EXCEEDED", cycle, ...this.performanceBudget.snapshot(), error: String(error) })); return { status: "blocked", cycles: cycle - 1, history, reason: String(error) }; }
            ...
            if (decision.kind === "platform-complete") {
                ...
                console.log(JSON.stringify({ type: "AUTONOMOUS_PLATFORM_CONSTRUCTION_COMPLETE", cycle, status: "completed", assistantComplete: true, autonomousConstructionComplete: true, canonicalPlatformConstructionComplete: decision.canonicalAudit.complete, commercialProductRuntimeComplete: decision.commercialAudit.complete, externalProductionDependenciesComplete: decision.commercialAudit.blockedExternalDependencies.length === 0, productComplete: true, backlogExhausted: decision.canonicalAudit.backlogExhausted, nonAutonomousProductionItems: decision.canonicalAudit.nonAutonomousProductionItems, commercialMissingLayers: decision.commercialAudit.missingLayers, blockedExternalDependencies: [], continuation: decision.continuation, performance: budgetSnapshot, message: "Assistant, canonical construction, commercial runtime and external production dependencies are complete." }));
            ...
            if (!weavingPlan.safe) { budgetSnapshot = this.performanceBudget.completeCycle(cycleStartedAt); return { status: "blocked", cycles: cycle, history: [...history, { cycle, weavingPlan, status: "blocked", performance: budgetSnapshot }] }; }
            ...
            if (!result.result.ok) {
                ...
                try { this.knotRecovery.rollback(this.root, recovery.checkpoint); console.log(JSON.stringify({ type: "AUTONOMOUS_ROLLBACK", cycle, checkpoint: recovery.checkpoint.commit, capabilityId: mission.capabilityId })); }
                catch (error) { budgetSnapshot = this.performanceBudget.completeCycle(cycleStartedAt); return { status: "blocked", cycles: cycle, history: [...history, { cycle, status: "blocked", result: { issues: ["CHECKPOINT_ROLLBACK_FAILED"], error: String(error) }, performance: budgetSnapshot }] }; }
                ...
                const repairResult = this.development.execute(repairGoal); history.push({ cycle, recovery, repairGoal, repairResult });
                if (!repairResult.result.ok) { budgetSnapshot = this.performanceBudget.completeCycle(cycleStartedAt); return { status: "blocked", cycles: cycle, history }; }
                result = repairResult;
                const repaired = this.mission.snapshot();
                if (repaired.commit === before.commit && !result.result.idempotent) { budgetSnapshot = this.performanceBudget.completeCycle(cycleStartedAt); return { status: "blocked", cycles: cycle, history }; }
                if (!repaired.clean) { budgetSnapshot = this.performanceBudget.completeCycle(cycleStartedAt); return { status: "blocked", cycles: cycle, history }; }
            }
            ...
            if (knotObservation.recover) { budgetSnapshot = this.performanceBudget.completeCycle(cycleStartedAt); return { status: "blocked", cycles: cycle, history }; }
            budgetSnapshot = this.performanceBudget.completeCycle(cycleStartedAt);
            const current = history[history.length - 1] as Record<string, unknown>; if (current) current.performance = budgetSnapshot;
            if (after.commit === before.commit && after.clean && !result.result.idempotent && cycle < this.maxCycles) return { status: "idle", cycles: cycle, history };
            if (result.result.idempotent) console.log(JSON.stringify({ type: "AUTONOMOUS_IDEMPOTENT_ADVANCE", cycle, capabilityId: mission.capabilityId, commit: after.commit }));
            if (this.reportEvery > 0 && cycle % this.reportEvery === 0) console.log(JSON.stringify({ type: "AUTONOMOUS_PROGRESS", cycle, commit: after.commit, clean: after.clean, performance: budgetSnapshot }));
        }
