...
export class AutonomousConstructionEngine {
    ...
    build(plan: ArchitecturePlan): ConstructionResult {
        ...
        if (generated && generated.changed === false && generated.idempotentNoOp !== true) {
            return this.blocked("VERIFY", attempt, ["QUALITY_IMPLEMENTATION_UNVERIFIED"], [...trace, "VERIFY"]);
        }
        ...
        if (!verification.ok) return this.blocked("VERIFY", attempt, [verification.issue || "VERIFICATION_FAILED"], trace);
        ...
        if (!finalize.ok) {
            ...
            else return this.blocked("FINALIZE", attempt, [finalize.issue || "FINALIZE_FAILED"], trace);
    ...
    private isIdempotentGenerationNoOp(result: { ok: boolean; artifact?: unknown; issue?: string }): boolean {
        ...
        const a = this.recordArtifact(result.artifact);
        return Boolean(a && a.changed === false && a.exitCode === 0 && typeof a.output === "string" && /Already implemented:/i.test(a.output));
    }
    private isIdempotentFinalizeNoOp(result: { ok: boolean; artifact?: unknown; issue?: string }, artifacts: Record<string, unknown>): boolean {
        ...
        return this.recordArtifact(artifacts.GENERATE)?.idempotentNoOp === true;
    }
    ...
    private toolFor(stage: ConstructionStage): ConstructionTool {
        const preferred: Record<ConstructionStage,string[]> = { ARCHITECTURE:["architecture"], PLAN:["architecture","planner"], GENERATE:["generator","python"], VERIFY:["python","verifier","test"], REPAIR:["python","repair"], FINALIZE:["git","finalizer"] };
        for (const name of preferred[stage]) { const tool=this.tools.find(c=>c.name===name); if(tool)return tool; }
