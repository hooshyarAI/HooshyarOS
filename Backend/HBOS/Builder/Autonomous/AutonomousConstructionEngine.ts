...
/** Architecture-driven construction control plane with fail-closed verification evidence. */
export class AutonomousConstructionEngine {
    ...
    build(plan: ArchitecturePlan): ConstructionResult {
        ...
        for (const stage of ["ARCHITECTURE", "PLAN", "GENERATE"] as ConstructionStage[]) {
            ...
            if (stage === "GENERATE") {
                const generated = this.recordArtifact(result.artifact);
                if (generated && generated.changed === false) return this.blocked("VERIFY", attempt, ["QUALITY_IMPLEMENTATION_UNVERIFIED"], [...trace, "VERIFY"]);
            }
        }
        trace.push("VERIFY"); issues.length = 0;
        let verification = this.execute("VERIFY", plan, attempt, artifacts, issues);
        verification = this.enforceVerificationEvidence(verification);
        while (!verification.ok && attempt < this.maxRepairAttempts) {
            ...
            trace.push("VERIFY"); issues.length = 0;
            verification = this.enforceVerificationEvidence(this.execute("VERIFY", plan, attempt, artifacts, issues));
    ...
    private enforceVerificationEvidence(result: { ok: boolean; artifact?: unknown; issue?: string }) {
        if (!result.ok) return result;
        const artifact = this.recordArtifact(result.artifact);
        if (!artifact) return result;
        if (artifact.behavioralEvidenceVerified === false) return { ...result, ok: false, issue: "QUALITY_BEHAVIOR_UNVERIFIED" };
        if (artifact.integrationVerified === false) return { ...result, ok: false, issue: "QUALITY_INTEGRATION_UNVERIFIED" };
        if (artifact.testsPassed === false || artifact.jestVerified === false) return { ...result, ok: false, issue: "QUALITY_TESTS_UNVERIFIED" };
        if (artifact.cleanRepository === false) return { ...result, ok: false, issue: "QUALITY_REPOSITORY_UNCLEAN" };
        return result;
    }
    private isIdempotentGenerationNoOp(result: { ok: boolean; artifact?: unknown; issue?: string }) { const a=this.recordArtifact(result.artifact); const o=typeof a?.output === "string" ? a.output : ""; return !result.ok && result.issue === "AUTONOMOUS_AGENT_NO_REPOSITORY_CHANGE" && a?.changed === false && a?.exitCode === 0 && /Already implemented:/i.test(o); }
    private isIdempotentFinalizeNoOp(result: { ok: boolean; artifact?: unknown; issue?: string }, artifacts: Record<string, unknown>) { return !result.ok && result.issue === "GIT_NO_REPOSITORY_CHANGE" && this.recordArtifact(artifacts.GENERATE)?.idempotentNoOp === true; }
    ...
    private toolFor(stage: ConstructionStage): ConstructionTool { const names: Record<ConstructionStage,string[]>={ARCHITECTURE:["architecture"],PLAN:["architecture","planner"],GENERATE:["generator","python"],VERIFY:["python","verifier","test"],REPAIR:["python","repair"],FINALIZE:["git","finalizer"]}; for(const n of names[stage]){const t=this.tools.find(x=>x.name===n);if(t)return t;} return this.tools[0]||{name:"unavailable",execute:()=>({ok:false,issue:"NO_CONSTRUCTION_TOOL"})}; }
    ...
    static selfTest():void{let calls=0;const tools:ConstructionTool[]=[{name:"architecture",execute:()=>({ok:true})},{name:"python",execute:stage=>{if(stage==="VERIFY"){calls++;return calls===1?{ok:false,issue:"INTERNAL_CONNECTION_FAILURE"}:{ok:true,artifact:{testsPassed:true,behavioralEvidenceVerified:true,integrationVerified:true,cleanRepository:true}};}return stage==="GENERATE"?{ok:true,artifact:{changed:true}}:{ok:true};}},{name:"git",execute:()=>({ok:true})}];const r=new AutonomousConstructionEngine(tools,2).build({capabilityId:"construction-001",capability:"architecture-driven autonomous construction",targetEngine:"Autonomous Operations Engine",dependencies:["Architecture Brain","Governance Engine"],architectureRules:["Architecture Freeze V4","One Capability = One Engine"]});if(!r.ok||r.status!=="REPAIRED"||r.attempts!==1||!r.details||!r.trace.includes("FINALIZE"))throw new Error("AutonomousConstructionEngine self-test failed");}
}
