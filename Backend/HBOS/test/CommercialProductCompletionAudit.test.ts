...
describe("CommercialProductCompletionAudit", () => {
    ...
    it("does not claim commercial completion when a deterministic behavioral quality failure exists", () => {
        ...
        try {
            ...
            writeFileSync(join(root, "Backend/HBOS/Product/CommercialE2EAcceptanceScenario.ts"), "export class CommercialE2EAcceptanceScenario {}\n", "utf8");
            writeFileSync(join(root, "Backend/HBOS/test/CommercialE2EAcceptanceScenario.test.ts"), "describe(\"CommercialE2EAcceptanceScenario\", () => { it(\"exists\", () => expect(true).toBe(true)); });\n", "utf8");
            writeFileSync(join(root, "Backend/HBOS/test/CommercialWebUXContract.test.ts"), "describe(\"CommercialWebUXContract\", () => { it(\"exists\", () => expect(true).toBe(true)); });\n", "utf8");
    ...
    it("reports missing application evidence instead of claiming completion", () => {
        const root = mkdtempSync(join(process.cwd(), ".tmp-commercial-evidence-audit-"));
        try {
            const contractSource = resolve(process.cwd(), "Docs/COMMERCIAL_PRODUCT_COMPLETION_CONTRACT.md");
            mkdirSync(join(root, "Docs/Product"), { recursive: true });
            mkdirSync(join(root, "Backend/HBOS/Infrastructure"), { recursive: true });
            ...
            writeFileSync(join(root, "Docs/Product/PRODUCT_CONSTRUCTION_ROADMAP.json"), JSON.stringify({ capabilities: [] }), "utf8");

            const result = new CommercialProductCompletionAudit().audit(root);
            expect(result.complete).toBe(false);
            expect(result.missingLayers).toContain("application-e2e-scenario");
            expect(result.missingLayers).toContain("application-e2e-test");
            expect(result.missingLayers).toContain("commercial-web-ux-contract");
        } finally {
            rmSync(root, { recursive: true, force: true });
        }
    });
