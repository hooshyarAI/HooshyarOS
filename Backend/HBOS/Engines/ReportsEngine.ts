...
export class ReportsEngine implements Engine {
    ...
    private header(document: ReportDocument): string[] {
        const metadataLabels: Record<string, string> = {
            ...
            "Source SHA-256": "شناسه صحت منبع (SHA-256)",
            "Generated by": "تولیدکننده",
        };
        const lines = [document.title, `سازمان: ${document.tenantId}`, `تاریخ تولید: ${document.generatedAt}`];
        for (const [key, value] of Object.entries(document.metadata ?? {})) {
            lines.push(`${metadataLabels[key] || key}: ${value}`);
        }
        return lines;
    }
    ...
    private renderCsv(document: ReportDocument): string {
        ...
        const rows: string[] = [`${escape("بخش")},${escape("محتوا")}`];
        for (const line of this.header(document)) rows.push(`${escape("گزارش")},${escape(line)}`);
        for (const section of document.sections) {
    ...
    private async renderXlsx(document: ReportDocument): Promise<Buffer> {
        ...
        sheet.columns = [
            { header: "بخش", key: "section", width: 30 },
            { header: "محتوا", key: "entry", width: 95 },
        ];
