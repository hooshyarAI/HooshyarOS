...
STRATEGIC RENEWAL / CONTINUOUS INNOVATION RULE:
Treat existing architecture, product, capability set, providers and assumptions as renewable. Do not assume a previously correct decision is permanently correct. When a mission involves replacing, retiring or re-evaluating an existing capability or provider, reason from measured evidence and prefer improve -> scale -> adapt -> replace -> retire rather than speculative rewrite. Legacy carries no automatic authority (anti-legacy-bias), but a retained decision that remains correct must not be discarded for novelty alone.
Where uncertainty is material, prefer a bounded, reversible experiment with an explicit hypothesis, success criteria, scope, stop conditions and measured result over an unmeasured rewrite, and never present an unmeasured experiment as completed capability. Experiments must not bypass architecture, security, tenant isolation or governance.
Label claims by epistemic status: FACT (authoritative evidence), ASSUMPTION (believed, unverified), HYPOTHESIS (falsifiable proposition), EXPERIMENT (bounded test), MEASURED RESULT (observed outcome), DECISION (governed choice). Never present an assumption or hypothesis as a fact or a measured result.
When renewal concerns an external capability, the permanent permitted class still applies: free + open source + licensed for commercial use + self-hosted/offline-suitable where required; an unverifiable license or commercial-use right remains `DEFERRED`, never assumed. This rule introduces no new engine, registry, subsystem or parallel governance and does not mutate Architecture Freeze V4/V4.1.
